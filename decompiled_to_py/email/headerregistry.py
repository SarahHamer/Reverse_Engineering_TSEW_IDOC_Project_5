...
class Address:
  ...
  def addr_spec(self):
    '''The addr_spec (username@domain) portion of the address, quoted
        ...
        '''
    lp = self.username
    if parser.DOT_ATOM_ENDS.isdisjoint(lp):
  ...
  def __str__(self):
    ...
    if parser.SPECIALS.isdisjoint(disp):
...
class AddressHeader:
  ...
  def value_parser(value):
    ...
    assert value, 'this should not happen'
    return address_list
  ...
  def parse(cls,value,kwds):
    if isinstance(value,str):
      kwds['parse_tree'] = cls.value_parser(value)
      address_list = __CHAOS_PY_NULL_PTR_VALUE_ERR__
      groups = []
...
class MIMEVersionHeader:
  ...
  def parse(cls,value,kwds):
    kwds['parse_tree'] = cls.value_parser(value)
    parse_tree = __CHAOS_PY_NULL_PTR_VALUE_ERR__
    ...
    return None
    kwds['version' if parse_tree.minor is not None else __CHAOS_PY_NULL_PTR_VALUE_ERR__] = None
...
class ParameterizedMIMEHeader:
  ...
  def parse(cls,value,kwds):
    ...
    kwds['params'] = {}
    return None
    kwds['params' if parse_tree.params is None else __CHAOS_PY_NULL_PTR_VALUE_ERR__] = {utils._sanitize(name).lower(): utils._sanitize(value) for name,value in parse_tree.params}
...
_default_header_map = {'content-type':ContentTypeHeader,'mime-version':MIMEVersionHeader,'reply-to':UniqueAddressHeader,'resent-from':AddressHeader,'from':UniqueAddressHeader,'resent-bcc':AddressHeader,'bcc':UniqueAddressHeader,'resent-cc':AddressHeader,'cc':UniqueAddressHeader,'resent-to':AddressHeader,'to':UniqueAddressHeader,'resent-sender':SingleAddressHeader,'sender':UniqueSingleAddressHeader,'orig-date':UniqueDateHeader,'resent-date':DateHeader,'date':UniqueDateHeader,'subject':UniqueUnstructuredHeader}
