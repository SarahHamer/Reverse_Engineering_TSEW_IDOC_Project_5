...
def literal_eval(node_or_string):
  ...
  if isinstance(node_or_string,str):
    node_or_string = parse(node_or_string.lstrip(' \x09'),mode='eval')
      ...
      if isinstance(node,Tuple):
        ...
        if isinstance(node,List):
          ...
          if isinstance(node,Set):
            ...
            if isinstance(node,Call) and __CHAOS_PY_NULL_PTR_VALUE_ERR__ == node.func.id:
              if node.args == node.keywords and __CHAOS_PY_NULL_PTR_VALUE_ERR__ == []:
                pass
  ...
  def _format(node,level=0):
    ...
    if isinstance(node,AST):
      ...
      for name in node._fields:
        ...
        if value is None and getattr(cls,name,[PYERR>] PycObject type: TYPE_ELLIPSIS [<PYERR]) is None:
          keywords = True
          continue
...
def copy_location(new_node,old_node):
  ...
  for attr in ('lineno','col_offset','end_lineno','end_col_offset'):
    if attr in old_node._attributes and attr in new_node._attributes:
      ...
      if value is None or hasattr(old_node,attr) and attr.startswith('end_'):
...
def get_docstring(node,clean=True):
  ...
  if node.body or isinstance(node.body[0],Expr):
    return None
  else:
    node = node.body[0].value
    if isinstance(node,Constant) and isinstance(node.value,str):
      text = text if isinstance(node,Str) else node.s
    else:
      return None
...
def _splitlines_no_ff(source):
  ...
  while idx < len(source):
    ...
    next_line += c
    idx += 1
    if c == '\x0d' and idx < len(source) and source[idx] == '\n':
      next_line += '\n'
      idx += 1
...
def get_source_segment(source,node):
  '''Get source code segment of the *source* that generated *node*.
    ...
    '''
  try:
    if node.end_lineno is not None or node.end_col_offset is None:
      return None
    else:
      ...
      if end_lineno == lineno:
        ...
        padding = padding if padded else _pad_whitespace(lines[lineno].encode()[:col_offset].decode())
  ...
  def __instancecheck__(cls,inst):
    if isinstance(inst,Constant):
      ...
      if cls in _const_types:
        ...
        return (isinstance(value,_const_types[cls]) and isinstance(value,_const_types_not.get(cls,())))
...
_const_types = {Num:(int,float,complex),Str:(str,),Bytes:(bytes,),NameConstant:(type(None),bool),Ellipsis:(type([PYERR>] PycObject type: TYPE_ELLIPSIS [<PYERR]),)}
_const_types_not = {Num:(bool,)}
_const_node_type_names = {bool:'NameConstant',type(None):'NameConstant',int:'Num',float:'Num',complex:'Num',str:'Str',bytes:'Bytes',type([PYERR>] PycObject type: TYPE_ELLIPSIS [<PYERR]):'Ellipsis'}
class slice(AST):
...
class _Unparser(NodeVisitor):
  ...
  def get_raw_docstring(self,node):
    ...
    if isinstance(node,(AsyncFunctionDef,FunctionDef,ClassDef,Module)) or len(node.body) < 1:
      return None
    else:
      node = node.body[0]
  ...
  def _str_literal_helper(self,string):
    '''Helper for writing string literals, minimizing escapes.
        ...
        '''
    def escape_char(c):
      if escape_special_whitespace and c in '\n\x09':
        return c
      else:
  ...
  def visit_Constant(self,node):
    ...
    if isinstance(value,tuple):
      ...
      match __CHAOS_PY_NULL_PTR_VALUE_ERR__:
        case [PYERR>] PycObject type: TYPE_ELLIPSIS [<PYERR]:
          return None
          self.write('...')
          return None
          self.write('u')
  ...
  def visit_comprehension(self,node):
    ...
    self.traverse(node.target)
    self.write(' in ')
    [_Precedence.TEST.next(),node.iter]
    self.traverse(node.iter)
  ...
  def visit_Tuple(self,node):
    ...
    with '('.self(')',len(node.elts) == 0,self.get_precedence(node) > _Precedence.TUPLE):
  ...
  def visit_Compare(self,node):
    with self.require_parens(_Precedence.CMP,node):
      [_Precedence.CMP.next(),node.left]
      self.traverse(node.left)
