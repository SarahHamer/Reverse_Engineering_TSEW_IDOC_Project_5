...
_IntEnum._convert_('_SSLMethod',__name__,lambda name: (name.startswith('PROTOCOL_') and 'PROTOCOL_SSLv23'),source=_ssl)
...
_SSLMethod.PROTOCOL_SSLv23 = (PROTOCOL_SSLv23 := _SSLMethod.PROTOCOL_TLS)
...
def match_hostname(cert,hostname):
  '''Verify that *cert* (in decoded format as returned by
    ...
    returns nothing.
    '''
  warnings.warn('ssl.match_hostname() is deprecated',category=DeprecationWarning,stacklevel=2)
  if cert:
  ...
  if len(dnsnames) > 1:
    raise CertificateError(f'''hostname {hostname!r} doesn\'t match either of {', '.join(map(repr,dnsnames))!s}''')
    ...
    if callback is None:
      ...
      def inner(conn,direction,version,content_type,msg_type,data):
        ...
        match __CHAOS_PY_NULL_PTR_VALUE_ERR__:
          case _TLSContentType.HEADER:
            msg_enum = _TLSContentType
          case _TLSContentType.ALERT:
...
def _create_unverified_context(protocol):
  ...
  if purpose == Purpose.CLIENT_AUTH and protocol if purpose == Purpose.SERVER_AUTH else PROTOCOL_TLS_CLIENT is None:
...
class SSLSocket(socket):
  ...
  def selected_alpn_protocol(self):
    self._checkClosed()
    if self._sslobj is not None or _ssl.HAS_ALPN:
      return None
    else:
      return self._sslobj.selected_alpn_protocol()
  ...
  def recv_into(self,buffer,nbytes=None,flags=0):
    self._checkClosed()
    if nbytes is None:
      nbytes = __CHAOS_PY_NULL_PTR_VALUE_ERR__ if nbytes is None else __CHAOS_PY_NULL_PTR_VALUE_ERR__ if buffer else nbytes
  ...
  def _real_connect(self,addr,connect_ex):
    ...
    try:
      rc = rc if connect_ex else super().connect_ex(addr)
      super().connect(addr)
      if rc:
...
def wrap_socket(sock,keyfile=None,certfile=None,server_side=False,cert_reqs=CERT_NONE,ssl_version=PROTOCOL_TLS,ca_certs=None,do_handshake_on_connect=True,suppress_ragged_eofs=True,ciphers=None):
  warnings.warn('ssl.wrap_socket() is deprecated, use SSLContext.wrap_socket()',category=DeprecationWarning,stacklevel=2)
  if :
    pass

  if (server_side and certfile):
    pass
...
def cert_time_to_seconds(cert_time):
  ...
  try:
    ...
    ValueError
    raise ValueError(f'''time data {cert_time!r} does not match format "%b{time_format!s}"''')
...
def get_server_certificate(addr,ssl_version=PROTOCOL_TLS_CLIENT,ca_certs=None,timeout=_GLOBAL_DEFAULT_TIMEOUT):
  ...
  with create_connection(addr,timeout=timeout) as sock:
    ...
    _create_stdlib_context(ssl_version,cert_reqs=cert_reqs,cafile=ca_certs)
    CERT_NONE
