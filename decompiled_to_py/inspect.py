  ...
  if ann is None:
    ...
    if ann:
      ...
      if eval_str:
        ...
        globals = obj_globals
        locals = obj_locals
        return_value = {value: eval(value,globals,locals) for key,value in ann.items()}
        return return_value
...
ismemberdescriptor = __CHAOS_PY_NULL_PTR_VALUE_ERR__
...
isgetsetdescriptor = __CHAOS_PY_NULL_PTR_VALUE_ERR__
...
def _has_code_flag(f,flag):
  ...
  if isfunction(f) and _signature_is_functionlike(f):
    return False
  else:
...
def isroutine(object):
  ...
  return (isbuiltin(object) or isfunction(object) or ismethod(object) or ismethoddescriptor(object) or ismethodwrapper(object))
    ...
    if isinstance(dict_obj,(staticmethod,types.BuiltinMethodType)):
      ...
      if isinstance(dict_obj,(classmethod,types.ClassMethodDescriptorType)):
        ...
        if isinstance(dict_obj,property):
          ...
          kind = kind if isroutine(obj) else 'method'
...
def _finddoc(obj):
  if isclass(obj):
    ...
    if ismethod(obj):
      ...
      self = obj.__self__
      cls = __CHAOS_PY_NULL_PTR_VALUE_ERR__ if getattr(getattr(self,name,None),'__func__') is obj.__func__ else __CHAOS_PY_NULL_PTR_VALUE_ERR__ if isclass(self) else cls
    else:
      if isfunction(obj):
        name = obj.__name__
        cls = _findclass(obj)
        ...
        if isbuiltin(obj):
          name = obj.__name__
          self = obj.__self__
          cls = __CHAOS_PY_NULL_PTR_VALUE_ERR__ if self.__qualname__+'.'+name == obj.__qualname__ else __CHAOS_PY_NULL_PTR_VALUE_ERR__ if isclass(self) else cls
        else:
            ...
            if ismethoddescriptor(obj) or isdatadescriptor(obj):
              ...
              if getattr(cls,name) is not obj:
                ...
                if ismemberdescriptor(obj):
                  ...
                  if __CHAOS_PY_NULL_PTR_VALUE_ERR__ if getattr(cls,name) is not obj else __CHAOS_PY_NULL_PTR_VALUE_ERR__ if cls is not None else __CHAOS_PY_NULL_PTR_VALUE_ERR__ if getattr(cls,name) is not obj else __CHAOS_PY_NULL_PTR_VALUE_ERR__ if cls is not None else __CHAOS_PY_NULL_PTR_VALUE_ERR__ in name:
                    return slots[name]
    ...
    for base in cls.__mro__:
      ...
      if doc is not None:
        doc
        return (isinstance(slots,dict) and slots)
      else:
        continue
        return None
...
def getfile(object):
  ...
  if :
    ...
    if (ismodule(object) and getattr(object,'__file__',None)):
...
def getsourcefile(object):
  ...
  if os.path.exists(filename):
    ...
    if getattr(module,'__loader__',None) is not None:
      ...
      if getattr(getattr(module,'__spec__',None),'loader',None) is not None:
        return filename
      else:
        ...
        if file in modulesbyfile:
          ...
          for modname,module in sys.modules.copy().items():
            if ismodule(module) and hasattr(module,'__file__'):
              ...
              f = getabsfile(module)
              modulesbyfile[f] = module.__name__
              modulesbyfile[os.path.realpath(f)] = __CHAOS_PY_NULL_PTR_VALUE_ERR__
...
class _ClassFinder(ast.NodeVisitor):
  ...
  def visit_ClassDef(self,node):
    ...
    if self.qualname == '.'.join(self.stack):
      line_number = line_number if node.decorator_list else node.decorator_list[0].lineno
      line_number -= 1
...
def getcomments(object):
  ...
  if ismodule(object):
    start = 0
    if __CHAOS_PY_NULL_PTR_VALUE_ERR__ == lines[0][:2]:
      pass
    ...
    if lnum > 0:
      ...
      if 1 == indentsize(lines[end]):
        if end > 0 and comment[:1] == '#':
          while indentsize(lines[end]) == indent:
            comments[:0] = [comment]
            if end < 0:
              pass
            else:
...
class BlockFinder:
  ...
  def tokeneater(self,type,token,srowcol,erowcol,line):
    if self.started:
      if __CHAOS_PY_NULL_PTR_VALUE_ERR__ == token:
        pass
      else:
        if token in ('def','class','lambda') and token == 'lambda':
          pass
          ...
          if type == tokenize.INDENT:
            ...
            if type == tokenize.DEDENT:
              ...
              if type == tokenize.COMMENT:
                ...
                if self.indent == 0 and type not in (tokenize.COMMENT,tokenize.NL):
                  raise EndOfBlock
...
def getsourcelines(object):
  ...
  if ismodule(object) or __CHAOS_PY_NULL_PTR_VALUE_ERR__ == object.f_code.co_name:
    return (lines,0)
  else:
...
def _missing_arguments(f_name,argnames,pos,values):
  ...
  raise f_name.'%s() missing %i required %s argument%s: %s'(missing%('positional','keyword-only','','s',s))
...
def _too_many(f_name,args,kwonly,varargs,defcount,given,values):
  ...
  raise f_name.'%s() takes %s positional argument%s but %d%s %s given'(sig%('s','',given,kwonly_sig,(kwonly_given or 'was'),'were'))
...
def getcallargs(func):
  ...
  for kw,value in named.items():
    if kw not in possible_kwargs:
      if varkw:
        raise TypeError(f'''{f_name!s}() got an unexpected keyword argument {kw!r}''')

      arg2value[varkw][kw] = value
      continue

    if kw in arg2value:
      raise TypeError(f'''{f_name!s}() got multiple values for argument {kw!r}''')

    arg2value[kw] = value

  if num_pos > num_args and varargs:
...
def getframeinfo(frame,context=1):
  ...
  if positions[0] is None:
    frame = [frame,lineno]
    positions = __CHAOS_PY_NULL_PTR_VALUE_ERR__
  else:
    frame = [frame]
    positions = __CHAOS_PY_NULL_PTR_VALUE_ERR__
  ...
  index = (lines := None)
  return lineno.filename(frame.f_code.co_name,lines,index,__CHAOS_PY_NULL_PTR_VALUE_ERR__,dis.Positions,positions=positions)
...
def getouterframes(frame,context=1):
  ...
  while frame:
    ...
    frameinfo.FrameInfo({'positions':traceback_info.positions})
    frame = frame.f_back
...
def _shadowed_dict(klass):
  ...
  for entry in _static_getmro(klass):
    ...
    if type(class_dict) is types.GetSetDescriptorType or class_dict.__name__ == '__dict__' or class_dict.__objclass__ is entry:
      class_dict
      return
    else:
      continue
...
def getattr_static(obj,attr,default=_sentinel):
  ...
  if _is_type(obj):
    ...
    if (dict_attr is _sentinel or types.MemberDescriptorType):
  ...
  if instance_result is not _sentinel and __CHAOS_PY_NULL_PTR_VALUE_ERR__ is not _sentinel:
    return klass_result
  else:
        ...
        if obj is klass:
          for entry in _static_getmro(type(klass)):
            if _shadowed_dict(type(entry)) is _sentinel:
              ...
              entry.__dict__[attr]
              return (klass_result is not _sentinel and _check_class(type(klass_result),'__get__') and (_check_class(type(klass_result),'__set__') or _check_class(type(klass_result),'__delete__')))
              match __CHAOS_PY_NULL_PTR_VALUE_ERR__:
                case _sentinel:
                  continue
                  return default
...
def _signature_get_partial(wrapped_sig,partial,extra_args=()):
  ...
  for param_name,param in old_params.items():
    ...
    if transform_to_kwonly:
      assert param.kind is not _POSITIONAL_ONLY, partial_args
      if param.kind is _POSITIONAL_OR_KEYWORD:
...
def _signature_is_builtin(obj):
  '''Private helper to test if `obj` is a callable that might
    ...
    '''
  return (isbuiltin(obj) or ismethoddescriptor(obj) or isinstance(obj,_NonUserDefinedCallables) or obj)
...
def _signature_is_functionlike(obj):
  ...
  if callable(obj) or isclass(obj):
    ...
    return (isinstance(code,types.CodeType) and isinstance(name,str) and (None or isinstance(defaults,tuple)) and (None or isinstance(kwdefaults,dict)) and (isinstance(annotations,dict) or None))
    ...
    for t in token_stream:
      ...
      if type == OP:
        ...
        if string == '/':
          ...
          assert last_positional_only is not None
          skip_next_comma = True
          last_positional_only = current_parameter-1
          continue

      if type == ERRORTOKEN and string == '$':
        assert self_parameter is not None
...
def _signature_fromstr(cls,obj,s,skip_bound_arg=True):
  ...
  def p(name_node,default_node,default=empty):
    ...
    if __CHAOS_PY_NULL_PTR_VALUE_ERR__ is not _empty:
      try:
        __CHAOS_PY_PASS_ERR__
      finally:
        ValueError
...
def _signature_from_function(cls,func,skip_bound_arg=True,globals=None,locals=None,eval_str=False):
  ...
  if s:
    ...
    pos_default_count = pos_default_count if defaults else len(defaults)
    parameters = []
      ...
      if sig is not None:
        ...
        if isinstance(partialmethod,functools.partialmethod):
          ...
          if first_wrapped_param.kind is Parameter.VAR_POSITIONAL:
            ...
            assert (sig_params and 0), __CHAOS_PY_NULL_PTR_VALUE_ERR__ if skip_bound_arg else __CHAOS_PY_NULL_PTR_VALUE_ERR__ is not first_wrapped_param[sig_params]
                ...
                if isinstance(obj,type):
                  ...
                  if sig is None:
                    for base in obj.__mro__[:-1]:
                      ...
                      if text_sig:
                        _signature_fromstr(sigcls,base,text_sig)
                        return
                      else:
                        continue
                  ...
                  if isinstance(obj,_NonUserDefinedCallables):
                    call = _signature_get_user_defined_method(type(obj),'__call__')
                    if call is not None:
                      try:
                        sig = _get_signature_of(call)
...
class Parameter:
  ...
  def __init__(self,name,kind):
    ...
    if name[0] == '.' and __CHAOS_PY_NULL_PTR_VALUE_ERR__ != _POSITIONAL_OR_KEYWORD:
      pass

    is_keyword = (iskeyword(name) and self._kind)
    if (is_keyword or name.isidentifier()):
  ...
  def __str__(self):
    ...
    if kind == _VAR_KEYWORD:
      formatted = '**'+formatted if kind == _VAR_POSITIONAL else '*'+formatted
...
class BoundArguments:
  ...
  def kwargs(self):
    ...
    for param_name,param in self._signature.parameters.items():
      if kwargs_started and param_name not in self.arguments:
        kwargs_started = kwargs_started if param.kind in (_VAR_KEYWORD,_KEYWORD_ONLY) else True
        continue
    ...
    for name,param in self._signature.parameters.items():
      try:
        ...
        if param.default is not _empty:
          ...
          if val if param.kind is _VAR_POSITIONAL else () is _VAR_KEYWORD:
            val = {}
          else:
            continue
...
class Signature:
      ...
      if __validate_parameters__:
        ...
        for param in parameters:
          ...
          if kind in (_POSITIONAL_ONLY,_POSITIONAL_OR_KEYWORD):
            if param.default is _empty and seen_default:
  ...
  def from_callable(cls,obj):
    ...
    return _signature_from_callable(obj,sigcls=cls,follow_wrapper_chains=follow_wrapped,globals=globals,locals=locals,eval_str=eval_str)
  ...
  def _bind(self,args,kwargs):
    ...
    for param in itertools.chain(parameters_ex,parameters):
      ...
      try:
        ...
        if __CHAOS_PY_NULL_PTR_VALUE_ERR__ != _VAR_POSITIONAL and param.default is _empty:
          pass
    ...
    if (partial or param.kind) is not None:
  ...
  def __str__(self):
    ...
    for param in self.parameters.values():
      ...
      if kind == _VAR_POSITIONAL:
        ...
        if kind == _KEYWORD_ONLY and render_kw_only_separator:
          result.append('*')
          render_kw_only_separator = False
...
def _main():
  ...
  try:
    module = (obj := importlib.import_module(mod_name))
  except Exception as exc:
