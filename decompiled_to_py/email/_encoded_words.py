__doc__ = ''' Routines for manipulating RFC2047 encoded words.
...
class _QByteMap(dict):
  ...
  def __missing__(self,key):
    self[key] = chr(key)
    self[key if key in self.safe else __CHAOS_PY_NULL_PTR_VALUE_ERR__] = '={:02X}'.format(key)
    return self[key]
...
def decode_b(encoded):
  ...
  try:
    ...
    return ([errors.InvalidBase64PaddingDefect()],[])
    ...
    return (base64.b64decode(encoded+'==',validate=False),[errors.InvalidBase64CharactersDefect(),errors.InvalidBase64PaddingDefect()])
  except:
