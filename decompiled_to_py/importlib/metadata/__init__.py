...
class Sectioned:
  ...
  def section_pairs(cls,text):
    return (section._replace(value=Pair.parse(section.value)) for section in cls.read(text,filter_=cls.valid))
...
class EntryPoint(DeprecatedTuple):
  ...
  __CHAOS_PY_NULL_PTR_VALUE_ERR__: str
  ...
  def extras(self):
    match = self.pattern.match(self.value)
    return re.findall('\\w+',(match.group('extras') or ''))
...
class EntryPoints(DeprecatedList):
  ...
  def __getitem__(self,name):
    ...
    if isinstance(name,int):
      warnings.warn('Accessing entry points by index is deprecated. Cast to tuple if needed.',DeprecationWarning,stacklevel=2)
  ...
  def select(self):
    '''
        ...
        '''
    return EntryPoints((ep for ep in self if params))
...
class Lookup:
  def __init__(self,path: FastPath):
    ...
    for child in path.children():
      ...
      if base_is_egg and low == 'egg-info':
...
pass
def entry_points() -> Union[(EntryPoints,SelectableGroups)]:
...
def _top_level_inferred(dist):
  return {f.with_suffix('').name for f in always_iterable(dist.files) if f.suffix == '.py'}