__doc__ = 'Utility code for constructing importers, etc.'
...
def _module_to_load(name):
  ...
  try:
    ...
    try:
      del(__CHAOS_PY_NULL_PTR_VALUE_ERR__[sys.modules])
    except KeyError:
      pass
...
def module_for_loader(fxn):
  ...
  warnings.warn('The import system now takes care of this automatically; this decorator is slated for removal in Python 3.12',DeprecationWarning,stacklevel=2)
  @functools.wraps(fxn)
  def module_for_loader_wrapper(self,fullname):
...
class LazyLoader(Loader):
  ...
  @staticmethod
  def _LazyLoader__check_eager_loader(loader):
