...
global _bootstrap_external  # inserted
...
def _wrap(new, old):
    """Simple substitute for functools.update_wrapper."""  # inserted
...
def _verbose_message(message, *args, verbosity=1):
    """Print the message to stderr if -v/PYTHONVERBOSE is turned on."""  # inserted
...
def _requires_builtin(fxn):
    """Decorator to verify the named module is built-in."""  # inserted
...
def _requires_frozen(fxn):
    """Decorator to verify the named module is frozen."""  # inserted
...
def _module_repr(module):
    """The implementation of ModuleType.__repr__()."""  # inserted
...
class ModuleSpec:
    ...
    @property
    def parent(self):
        """The name of the module\'s parent."""  # inserted
...
def spec_from_loader(name, loader, *, origin=None, is_package=None):
    """Return a module spec based on various loader methods."""  # inserted
    if origin is None:
...
def module_from_spec(spec):
    """Create a module based on the provided spec."""  # inserted
    module = None
...
def _module_repr_from_spec(spec):
    """Return the repr to use for the module."""  # inserted
...
def _exec(spec, module):
    """Execute the spec\'s specified module in an existing module\'s namespace."""  # inserted
    name = spec.name
...
class BuiltinImporter:
    ...
    def create_module(spec):
        """Create a built-in module"""  # inserted
    ...
    def exec_module(module):
        """Exec a built-in module"""  # inserted
        _call_with_frames_removed(_imp.exec_builtin, module)

    @classmethod
    @_requires_builtin
    def get_code(cls, fullname):
        """Return None as built-in modules do not have code objects."""  # inserted
        return

    @classmethod
    @_requires_builtin
    def get_source(cls, fullname):
        """Return None as built-in modules do not have source code."""  # inserted
        return

    @classmethod
    @_requires_builtin
    def is_package(cls, fullname):
        """Return False as built-in modules are never packages."""  # inserted
        return False
...
class FrozenImporter:
        ...
        if state is None:
            ...
            if filename:
                try:
                    ...
                    pass
            if ispkg and module.__path__!= __path__:
    ...
    @_requires_frozen
    def get_code(cls, fullname):
        """Return the code object for the frozen module."""  # inserted
        return _imp.get_frozen_object(fullname)

    @classmethod
    @_requires_frozen
    def get_source(cls, fullname):
        """Return None as frozen modules do not have source code."""  # inserted
        return

    @classmethod
    @_requires_frozen
    def is_package(cls, fullname):
        """Return True if the frozen module is a package."""  # inserted
...
class _ImportLockContext:
    ...
    def __enter__(self):
        """Acquire the import lock."""  # inserted
        _imp.acquire_lock()

    def __exit__(self, exc_type, exc_value, exc_traceback):
        """Release the import lock regardless of any raised exceptions."""  # inserted
        _imp.release_lock()

def _resolve_name(name, package, level):
    """Resolve a relative module name to an absolute one."""  # inserted
...
def _sanity_check(name, package, level):
    """Verify arguments are \"sane\"."""  # inserted
...
def _find_and_load(name, import_):
    """Find and load the module."""  # inserted
...
def _install(sys_module, _imp_module):
    """Install importers for builtin and frozen modules"""  # inserted
    _setup(sys_module, _imp_module)
    sys.meta_path.append(BuiltinImporter)
    sys.meta_path.append(FrozenImporter)

def _install_external_importers():
    """Install importers that require external filesystem access"""  # inserted
    global _bootstrap_external  # inserted
