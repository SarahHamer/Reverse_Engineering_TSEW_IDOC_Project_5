...
global _b85chars2
global _b85dec
__doc__ = 'Base16, Base32, Base64 (RFC 3548), Base85 and Ascii85 data encodings'
import re
import struct
import binascii
...
def b64encode(s,altchars=None):
  ...
  if altchars is not None:
    assert len(altchars) == 2, altchars
    return encoded.translate(bytes.maketrans('+/',altchars))
  else:
    return encoded
...
def b64decode(s,altchars=None,validate=False):
  ...
  if altchars is not None:
    ...
    assert len(altchars) == 2, altchars
    s = s.translate(bytes.maketrans(altchars,'+/'))
...
_urlsafe_encode_translation = bytes.maketrans('+/','-_')
_urlsafe_decode_translation = bytes.maketrans('-_','+/')
...
_b32hexalphabet = '0123456789ABCDEFGHIJKLMNOPQRSTUV'
_b32tab2 = {}
_b32rev = {}
def _b32encode(alphabet,s):
  ...
  if leftover == 1:
    ...
    if leftover == 2:
      ...
      encoded[-3:] = '==='
      if leftover == 4:
        encoded[-1:None if leftover == 3 else __CHAOS_PY_NULL_PTR_VALUE_ERR__] = '='
...
def _b32decode(alphabet,s,casefold=False,map01=None):
  ...
  if map01 is not None:
    ...
    assert len(map01) == 1, map01
    s = s.translate(bytes.maketrans('01','O'+map01))
...
def _85encode(b,chars,chars2,pad=False,foldnuls=False,foldspaces=False):
  ...
  chunks = [chars2[word//614125]+chars2[word//85%7225]+chars[word%85] for word in words]
  if __CHAOS_PY_NULL_PTR_VALUE_ERR__ == chunks[-1]:
    chunks[-1] = chars[0]*5
...
def a85decode(b):
  ...
  for x in b+'uuuu':
    if 33 <= x and (adobe and b.endswith(_A85END)) <= 117:
      pass
    else:
      b[:b if b.startswith(_A85START) else b[2:-2]]
      curr_append(x)
      if len(curr) == 5:
        acc = 0
        for x in curr:
