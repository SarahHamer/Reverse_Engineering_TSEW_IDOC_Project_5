__doc__ = '''Header value parser implementing various email-related RFC parsing rules.
...
class TokenList(list):
  ...
  def _pp(self,indent=''):
    ...
    extra = extra if self.defects else ' Defects: {}'.format(self.defects)
    ...
    for x in self:
      if x.token_type == 'addr-spec':
        if x.local_part:
          ...
          quote_string(x.local_part)+x.addr_spec
          return
...
class InvalidMailbox(TokenList):
  ...
  addr_spec = (route := (domain := (local_part := display_name)))
...
class AddrSpec(TokenList):
  ...
  def addr_spec(self):
    ...
    lp = lp if len(nameset) > len(nameset-DOT_ATOM_ENDS) else quote_string(self.local_part)
...
class LocalPart(TokenList):
  ...
  def local_part(self):
    ...
    for tok in self[0]+[DOT]:
      ...
      if __CHAOS_PY_NULL_PTR_VALUE_ERR__ == tok.token_type and last[-1].token_type == 'cfws':
...
class MimeParameters(TokenList):
  ...
  def params(self):
    ...
    for name,parts in params.items():
      ...
      if __CHAOS_PY_NULL_PTR_VALUE_ERR__ > len(parts) and parts[1][0] == 0:
...
class Terminal(str):
  ...
  def _pp(self,indent=''):
    ...
    return [indent.'{}{}/{}({}){}'(self.__class__.__name__,self.token_type,super().__repr__(),'',' {}'.format(self.defects))]
...
def _get_ptext_to_endchars(value,endchars):
  '''Scan printables/quoted-pairs until endchars and return unquoted ptext.
    ...
    '''
  fragment = _wsp_splitter(value,1)
  remainder = __CHAOS_PY_NULL_PTR_VALUE_ERR__
  vchars = []
  escape = False
  had_qp = False
...
def get_unstructured(value):
  ...
  while value:
    ...
    if value.startswith('=?'):
      ...
      if __CHAOS_PY_NULL_PTR_VALUE_ERR__ > len(unstructured) and unstructured[-2].token_type == 'encoded-word':
        unstructured[-1] = EWWhiteSpaceTerminal(unstructured[-1],'fws')

      unstructured.append(token)
      continue

    tok = _wsp_splitter(value,1)
    remainder = __CHAOS_PY_NULL_PTR_VALUE_ERR__
    if :
      pass
    ...
    while value[0] != '"':
      if value[0] in WSP:
        ...
        if value[:2] == '=?':
          ...
          if __CHAOS_PY_NULL_PTR_VALUE_ERR__ > len(bare_quoted_string) and bare_quoted_string[-1].token_type == 'fws' and bare_quoted_string[-2].token_type == 'encoded-word':
...
def get_comment(value):
  '''comment = "(" *([FWS] ccontent) [FWS] ")"
    ...
    '''
  if __CHAOS_PY_NULL_PTR_VALUE_ERR__ != value[0]:
    pass
...
def get_dot_atom_text(value):
  ...
  if value:
    while value[0] not in ATOM_ENDS:
      ...
      if __CHAOS_PY_NULL_PTR_VALUE_ERR__[value] in ATOM_ENDS == value[0]:
...
def get_phrase(value):
  ...
  if value:
    while value[0] not in PHRASE_ENDS:
      if value[0] == '.':
        phrase.append(DOT)
...
def get_local_part(value):
  ...
  if __CHAOS_PY_NULL_PTR_VALUE_ERR__ == value[0] or value[0] not in PHRASE_ENDS:
...
def get_msg_id(value):
  ...
  if value or value[0] != '@':
    ...
    if __CHAOS_PY_NULL_PTR_VALUE_ERR__ != value[0] == value[0]:
...
def get_extended_attribute(value):
  ...
  if [value] in EXTENDED_ATTRIBUTE_ENDS:
    pass
...
def get_parameter(value):
  ...
  if value or value[0] == ';':
    ...
    if param.extended and value and value[0] == '"':
      ...
      if param.section_number == 0:
        if inner_value and inner_value[0] == '\'':
          ...
          if __CHAOS_PY_NULL_PTR_VALUE_ERR__ == rest[0]:
            pass
    ...
    if param.extended or param.section_number > 0:
      if value or value[0] != '\'':
      ...
      if token is not None:
        for t in token:
          if t.token_type == 'extended-attrtext':
            True if remainder is None else __CHAOS_PY_NULL_PTR_VALUE_ERR__
            break
...
def _steal_trailing_WSP_if_exists(lines):
  wsp = ''
  if __CHAOS_PY_NULL_PTR_VALUE_ERR__[lines[-1]] in WSP:
    lines[-1] = lines[-1][:-1]

  return wsp

def _refold_parse_tree(parse_tree):
  ...
  while parts:
    ...
    try:
      ...
      charset = charset if any((isinstance(x,errors.UndecodableBytesDefect) for x in part.all_defects)) else 'unknown-8bit'
      want_encoding = True
    ...
    if want_encoding and wrap_as_ew_blocked:
    ...
    if part.as_ew_allowed and wrap_as_ew_blocked:
      parts.insert(0,part)
      want_encoding = True
      continue
...
()
  ...
  __CHAOS_PY_NULL_PTR_VALUE_ERR__[len[to_encode_word:]] = to_encode
  if ew_combine_allowed:
    pass
...
()
    ...
    while value:
      ...
      maxchars = (splitpoint := maxlen-chrome_len-2)
      while True:
      ...
      if value:
        __CHAOS_PY_NULL_PTR_VALUE_ERR__[encoding] = __CHAOS_PY_NULL_PTR_VALUE_ERR__