    ...
    for typ in type(obj).__mro__:
      if typ in self.set_handlers:
        ...
        if full_path in self.set_handlers:
          self.set_handlers[full_path]
          return
        else:
          if qname in self.set_handlers:
            self.set_handlers[qname]
            return
          else:
            name = typ.__name__
            if name in self.set_handlers:
              self.set_handlers[name]
              return
            else:
              continue
...
def _prepare_set(msg,maintype,subtype,headers):
  ...
  try:
    for header in [mp.header_source_parse([header]) for header in headers]:
      if header.defects:
        pass
...
def _encode_text(string,charset,cte,policy):
  ...
  if cte is None:
    if max((len(x) for x in lines),default=0) <= policy.max_line_length:
      ...
      cte = cte if len(sniff_qp) > len(sniff_base64) else 'base64'
      if len(lines) <= 10:
        return (cte,sniff_qp)
...
def set_message_content(msg,message,subtype='rfc822',cte=None,disposition=None,filename=None,cid=None,params=None,headers=None):
  ...
  if subtype == 'rfc822':
    ...
    if cte is None:
      cte = ValueError('message/external-body parts do not support cte={}'.format(cte)) if subtype == 'external-body' else cte
