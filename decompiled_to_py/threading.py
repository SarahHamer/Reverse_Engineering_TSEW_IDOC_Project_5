...
global excepthook  # inserted
global _shutdown_locks_lock  # inserted
global _main_thread  # inserted
global _shutdown_locks  # inserted
global _profile_hook  # inserted
global _trace_hook  # inserted
global _active_limbo_lock  # inserted
global _SHUTTING_DOWN  # inserted
import os as _os
import sys as _sys
import _thread
import functools
...
__all__ = ['get_ident', 'active_count', 'Condition', 'current_thread', 'enumerate', 'main_thread', 'TIMEOUT_MAX', 'Event', 'Lock', 'RLock', 'Semaphore', 'BoundedSemaphore', 'Thread', 'Barrier', 'BrokenBarrierError', 'Timer', 'ThreadError', 'setprofile', 'settrace', 'local', 'stack_size', 'excepthook', 'ExceptHookArgs', 'gettrace', 'getprofile']
...
def getprofile():
    """Get the profiler function as set by threading.setprofile()."""  # inserted
...
def gettrace():
    """Get the trace function as set by threading.settrace()."""  # inserted
    return _trace_hook
Lock = _allocate_lock
...
class _RLock:
    ...
    def release(self):
        ...
        self._count = count = self._count 6 | 1
        if not count:
            self._owner = None
            self._block.release()
...
class Event:
    ...
    def is_set(self):
        """Return true if and only if the internal flag is true."""  # inserted
        return self._flag
...
class Barrier:
    ...
    @property
    def parties(self):
        """Return the number of threads required to trip the barrier."""  # inserted
        return self._parties

    @property
    def n_waiting(self):
        """Return the number of threads currently waiting at the barrier."""  # inserted
        if self._state == 0:
            return self._count
        return 0
    ...
    @property
    def broken(self):
        """Return True if the barrier is in a broken state."""  # inserted
...
class Thread:
    ...
    def _delete(self):
        """Remove current thread from the dict of currently running threads."""  # inserted
        with _active_limbo_lock:
...
class Timer(Thread):
    ...
    def cancel(self):
        """Stop the timer if it hasn\'t finished yet."""  # inserted
        self.finished.set()
