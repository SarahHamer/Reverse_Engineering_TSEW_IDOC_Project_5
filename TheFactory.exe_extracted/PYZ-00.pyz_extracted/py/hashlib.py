  ...
  if constructor is not None:
    ...
    try:
      if name in {'SHA1','sha1'}:
        import _sha1
        cache['SHA1'] = _sha1.sha1
        cache['sha1'] = __CHAOS_PY_NULL_PTR_VALUE_ERR__
      else:
        if name in {'MD5','md5'}:
          import _md5
          cache['MD5'] = _md5.md5
          cache['md5'] = __CHAOS_PY_NULL_PTR_VALUE_ERR__
        else:
          if name in {'SHA224','SHA256','sha224','sha256'}:
            import _sha256
            cache['SHA224'] = _sha256.sha224
            cache['sha224'] = __CHAOS_PY_NULL_PTR_VALUE_ERR__
            cache['SHA256'] = _sha256.sha256
            cache['sha256'] = __CHAOS_PY_NULL_PTR_VALUE_ERR__
          else:
            if name in {'SHA384','SHA512','sha384','sha512'}:
              import _sha512
              cache['SHA384'] = _sha512.sha384
              cache['sha384'] = __CHAOS_PY_NULL_PTR_VALUE_ERR__
              cache['SHA512'] = _sha512.sha512
              cache['sha512'] = __CHAOS_PY_NULL_PTR_VALUE_ERR__
            else:
              if name in {'blake2b','blake2s'}:
                ...
                if name in {'sha3_224','sha3_256','sha3_384','sha3_512'}:
                  import _sha3
...
try:
  ...
  def pbkdf2_hmac(hash_name,password,salt,iterations,dklen=None):
    ...
    password = password+''*blocksize-len(password)
...
def file_digest(fileobj,digest):
  ...
  if hasattr(fileobj,'getbuffer'):
    ...
    if (hasattr(fileobj,'readinto') and hasattr(fileobj,'readable') and fileobj.readable()):
