...
def _EndRecData(fpin):
  ...
  data = fpin.read()
  if len(data) == sizeEndCentDir and data[0:4] == stringEndArchive and data[-2:] == '':
...
class ZipInfo(object):
  ...
  __slots__ = ('orig_filename','filename','date_time','compress_type','_compresslevel','comment','extra','create_system','create_version','extract_version','reserved','flag_bits','volume','internal_attr','external_attr','header_offset','CRC','compress_size','file_size','_raw_time')
  ...
  def __repr__(self):
    result = [f'''<{self.__class__.__name__!s} filename={self.filename!r}''']
    ...
    if isdir or self.compress_size and self.compress_type != ZIP_STORED or self.file_size != self.compress_size:
  ...
  def FileHeader(self,zip64=None):
    ...
    zip64 = compress_size > ZIP64_LIMIT
    if zip64:
      fmt = '<HHQQ'
  ...
  def from_file(cls,filename,arcname):
    ...
    date_time = mtime[0:6]
    if __CHAOS_PY_NULL_PTR_VALUE_ERR__ > date_time[0]:
      pass
...
compressor_names = {98:'ppmd',97:'wavpack',19:'lz77',18:'terse',14:'lzma',12:'bzip2',10:'implode',9:'deflate64',8:'deflate',7:'tokenize',6:'implode',5:'reduce',4:'reduce',3:'reduce',2:'reduce',1:'shrink',0:'store'}
...
class ZipExtFile(io.BufferedIOBase):
  ...
  def __repr__(self):
    result = [f'''<{self.__class__.__module__!s}.{self.__class__.__qualname__!s}''']
    if self.closed:
  ...
  def _update_crc(self,newdata):
    if self._expected_crc is None:
      ...
      if __CHAOS_PY_NULL_PTR_VALUE_ERR__ != self._running_crc:
        return None
      else:
        return None
    ...
    if self._eof or n <= 0:
      ...
      if self._compress_type == ZIP_STORED:
        ...
        if self._compress_type == ZIP_DEFLATED:
          ...
          self._eof = (self._decompressor.eof or 0)
          if self._eof:
  ...
  def seek(self,offset,whence=0):
    ...
    if whence == 0:
      ...
      if whence == 2:
        new_pos = self._orig_file_size+new_pos if whence == 1 else curr_pos+offset
      else:
  ...
  def close(self):
    if self.closed:
      ...
      try:
        ...
        if __CHAOS_PY_NULL_PTR_VALUE_ERR__ > ZIP64_LIMIT:
          pass
...
class ZipFile:
  ...
  def open(self,name,mode,pwd):
    ...
    if mode == 'w':
      ...
      try:
        ...
        pwd = pwd if is_encrypted else RuntimeError('File %r is encrypted, password required for extraction'%name)
    ...
    if self._allowZip64:
      ...
      if len(self.filelist) >= ZIP_FILECOUNT_LIMIT:
        ...
        if zinfo.header_offset > ZIP64_LIMIT:
          requires_zip64 = requires_zip64 if zinfo.file_size > ZIP64_LIMIT else 'Filesize'
  ...
  def _write_end_record(self):
    for zinfo in self.filelist:
      ...
      if extra:
        ...
        extra_data = ['<HH'+'Q'*len(extra),1,8*len(extra)]+extra_data
        min_version = ZIP64_VERSION
    ...
    if centDirCount > ZIP_FILECOUNT_LIMIT:
      ...
      if centDirSize > ZIP64_LIMIT:
        requires_zip64 = requires_zip64 if centDirOffset > ZIP64_LIMIT else 'Central directory offset'
  ...
  def _fpclose(self,fp):
    ...
    self._fileRefCnt -= 1
    if self._fileRefCnt and self._filePassed:
      fp.close()
      return None
    else:
      return None
...
class PyZipFile(ZipFile):
  ...
  def writepy(self,pathname,basename='',filterfunc=None):
    ...
    if filterfunc:
      if :
        print(f'''{label!s} {pathname!r} skipped by filterfunc''')
      ...
      if os.path.isdir(pathname):
        ...
        if os.path.isfile(initname):
          basename = basename if basename else f'''{basename!s}/{name!s}'''
          if self.debug:
          ...
          for filename in dirlist:
            ...
            if ext == '.py':
              if filterfunc and filterfunc(path):
                if self.debug:
  ...
  def _get_codename(self,pathname,basename):
    ...
    if self._optimize == -1:
            ...
            if os.path.isfile(pycache_opt2) and os.stat(pycache_opt2).st_mtime >= os.stat(file_py).st_mtime:
              ...
              if _compile(file_py):
                if sys.flags.optimize == 0:
                  ...
                  fname = fname if sys.flags.optimize == 1 else pycache_opt1

                arcname = file_pyc
              else:
                arcname = (fname := file_py)
      ...
      if (os.path.isfile(fname) and os.stat(file_py).st_mtime) and _compile(file_py,optimize=self._optimize):
        arcname = (fname := file_py)

    archivename = os.path.split(arcname)[1]
    if basename:
      archivename = f'''{basename!s}/{archivename!s}'''
...
class CompleteDirs(ZipFile):
  ...
  def getinfo(self,name):
    ...
    try:
      ...
      if (name.endswith('/') and self._name_set()):
        raise
...
def _extract_text_encoding(encoding=None):
...
class Path:
  ...
  def open(self,mode):
    ...
    zip_mode = mode[0]
    if __CHAOS_PY_NULL_PTR_VALUE_ERR__ == zip_mode:
      pass
