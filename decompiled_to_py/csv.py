...
__all__ = ['QUOTE_MINIMAL','QUOTE_ALL','QUOTE_NONNUMERIC','QUOTE_NONE','Error','Dialect','__doc__','excel','excel_tab','field_size_limit','reader','writer','register_dialect','get_dialect','list_dialects','Sniffer','unregister_dialect','__version__','DictReader','DictWriter','unix_dialect']
class Dialect:
...
class DictReader:
  pass
  pass
  def __init__(self,f,fieldnames=None,restkey=None,restval=None,dialect='excel'):
...
class DictWriter:
  pass
  pass
  def __init__(self,f,fieldnames,restval='',extrasaction='raise',dialect='excel'):
  ...
  def _guess_quote_and_delimiter(self,data,delimiters):
    ...
    if matches:
      ...
      for m in matches:
        ...
        if __CHAOS_PY_NULL_PTR_VALUE_ERR__ in key:
      ...
      doublequote = doublequote if dq_regexp.search(data) else True
    ...
    while start < len(data):
      ...
      if len(delims) == 0:
        while consistency >= threshold:
          for k,v in modeList:
            if v[0] > 0 and v[1] > 0 and v[1]/total >= consistency and k in delimiters:
              delims[k] = v
